...
sed -i "s/VERSION=\"[^\"]*\"/VERSION=\"R$(TZ=Asia/Shanghai date '+%m.%d' | sed 's/^0//;s/\.0/./g')\"/" package/base-files/files/usr/lib/os-release
...
sed -i "s/DISTRIB_REVISION='[^']*'/DISTRIB_REVISION='R$(TZ=Asia/Shanghai date '+%m.%d' | sed 's/^0//;s/\.0/./g')'/" package/base-files/files/etc/openwrt_release
