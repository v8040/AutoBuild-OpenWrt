...
git clone -q --depth=1 'https://github.com/Zerogiven-OpenWRT-Packages/luci-app-podman.git' 'package/luci-app-podman'
...
sparse_clone 'master' 'https://github.com/immortalwrt/luci.git' 'applications/luci-app-lxc'
...
sparse_clone 'master' 'https://github.com/immortalwrt/packages.git' 'utils/lxc'
sparse_clone 'master' 'https://github.com/immortalwrt/packages.git' 'utils/rpcd-mod-lxc'
...
sed -i '/^LUCI_DEPENDS[[:space:]]*:=/{/+podman/!s/$/ +podman/}' package/luci-app-podman/Makefile
...
jq_menu 'feeds/luci/applications/luci-app-dockerman/root/usr/share/luci/menu.d/luci-app-dockerman.json' \
  'with_entries(
    .key |= sub("admin/services/dockerman"; "admin/docker") |
    if .key == "admin/docker" then .value += {title: "Docker", order: 35} else . end |
    if .value.action.path then .value.action.path |= sub("admin/services/dockerman"; "admin/docker") else . end
  )'
jq_menu 'package/luci-app-podman/root/usr/share/luci/menu.d/luci-app-podman.json' \
  '.["admin/podman"].order = 36'
