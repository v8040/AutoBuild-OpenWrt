...
sparse_clone() {
  ...
  if [[ -d "${TARGET_DIR}" ]]; then
    find "${TARGET_DIR}" -maxdepth 3 -name Makefile -exec sed -i \
      ...
      -e 's|\.\./\.\./lang/rust/rust-package\.mk|$(TOPDIR)/feeds/packages/lang/rust/rust-package.mk|g' \
      -e 's|\.\./\.\./lang/rust/rust-values\.mk|$(TOPDIR)/feeds/packages/lang/rust/rust-values.mk|g' \
      -e 's|\.\./\.\./lang/rust/rust-host-build\.mk|$(TOPDIR)/feeds/packages/lang/rust/rust-host-build.mk|g' \
...
jq_menu() {
  ...
  [[ -z "${FILE}" || -z "${FILTER}" ]] && return 1
  [[ -f "${FILE}" ]] || { warning "Menu file not found [${FILE}]"; return 1; }
  command -v jq &>/dev/null || { warning "jq not installed, skipping [${FILE}]"; return 1; }
  local RES
  if ! RES="$(jq "${FILTER}" "${FILE}" 2>/dev/null)"; then
    warning "jq failed on [${FILE}]"
    return 1
  fi
  [[ -n "${RES}" ]] || { warning "jq produced empty output for [${FILE}]"; return 1; }
  printf '%s\n' "${RES}" > "${FILE}"
  info "Updated menu [${FILE}]"
}
