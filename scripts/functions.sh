...
sparse_clone() {
  ...
  local CACHE_DIR="package/cache"
  [[ -d "${CACHE_DIR}" ]] && rm -rf "${CACHE_DIR}"
  git clone -q --branch="${BRANCH}" --depth=1 --filter=blob:none --sparse "${REPOURL}" "${CACHE_DIR}" &>/dev/null &&
  git -C "${CACHE_DIR}" sparse-checkout set "${REPODIR}" &>/dev/null &&
  mv -f "${CACHE_DIR}/${REPODIR}" package &&
  rm -rf "${CACHE_DIR}" || warning "Failed to sparse clone ${REPODIR} from ${REPOURL}(${BRANCH})"
}
